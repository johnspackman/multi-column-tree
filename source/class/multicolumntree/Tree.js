 ...
 */
qx.Class.define("multicolumntree.Tree", {
  ...
  construct: function() {
    ...
    this._setLayout(new multicolumntree.TreeLayout());
    
    this.bind("view.columns", new multicolumntree.TargetArray(this.__onColumnsChange, this), "value");
  ...
  members: {
    ...
    replaceSelection: function(arr) {
      arr = multicolumntree.Utils.toNativeArray(arr);
      var sel = this.getSelection();
    ...
    _getRowFromEvent: function(evt) {
      ...
      while (target && target != this) {
        if (qx.Class.isSubClassOf(target.constructor, multicolumntree.Row))
          return target;
    ...
    _getDragSource: function(evt) {
      ...
      var result = null;
      if (source instanceof multicolumntree.Row)
    ...
    __onDrop: function(evt) {
      ...
      return model.promiseGetChildren(dropParentNode)
        ...
        try {
          if (dropNode || !(sourceRow instanceof multicolumntree.Row)) {
            this.__caretOff();
            return;
          }
